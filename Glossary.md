...
1. [20 Software Engineering Laws](#20-software-engineering-laws)
...
References: [Milan Milanović - *The 20 Software Engineering Laws*](https://newsletter.techworld-with-milan.com/p/the-20-software-engineering-laws) | [Melvin Conway - *How Do Committees Invent?* (Datamation, 1968)](https://www.melconway.com/Home/Committees_Paper.html) | [Martin Fowler - *Conway's Law*](https://martinfowler.com/bliki/ConwaysLaw.html)

🔹 These laws are descriptive, not prescriptive: they help explain what is already happening in projects, teams, systems, and decision-making.

🔹 **1. How systems get built**
1. **Gall's Law**: Complex systems that succeed usually evolve from simple systems that already proved themselves in the real world.
1. **KISS**: Simplicity lowers maintenance cost, onboarding time, and bug surface area; extra cleverness is usually overhead.
1. **Conway's Law**: System boundaries tend to mirror team communication boundaries; if frontend, backend, data, and platform teams communicate poorly, the system often reflects those same awkward seams. Rewriting the system without changing the team structure usually reproduces the same architecture in a new stack. The inverse move is **Inverse Conway's Maneuver**: shape teams around the architecture you want, for example small autonomous teams aligned to clear service or domain boundaries.
1. **Hyrum's Law**: At scale, users depend on all observable behavior, including accidental behavior, not only the official contract.
1. **CAP Theorem**: During partitions, distributed systems must choose whether to favor consistency or availability.  
   → see [Principles & Concepts: YAGNI, KISS, DRY, CAP, PACELC, ACID, BASE](#principles-concepts-yagni-kiss-dry-cap-pacelc-acid-base)
1. **Zawinski's Law**: Useful software tends to absorb adjacent features until feature creep and bloat create an opening for simpler competitors.

🔹 **2. How teams lose speed**
1. **Brooks's Law**: Adding people to a late project usually slows it further because training and coordination increase nonlinearly.
1. **Ringelmann Effect**: As teams grow, output per person falls because coordination friction and social loafing increase.
1. **Price's Law**: A small fraction of contributors often carries a large share of important output, while supporting "glue work" still remains essential.

🔹 **3. Why plans drift**
1. **Hofstadter's Law**: Work takes longer than expected even after you account for the fact that estimates are usually optimistic.
1. **Dunning-Kruger Effect**: People with less skill often overestimate their ability because they lack the judgment needed to see their own gaps.
1. **Parkinson's Law**: Work expands to fill the time available, which is why loose timelines often invite gold-plating and delay.

🔹 **4. How metrics distort work**
1. **Goodhart's Law**: When a measure becomes a target, people optimize the number instead of the outcome it was meant to represent.
1. **Gilb's Law**: Even imperfect proxies are usually better than no measurement, as long as you remember they are proxies.

🔹 **5. What breaks under load**
1. **Knuth's Optimization Principle**: Premature optimization wastes time and adds complexity before real bottlenecks are known.
1. **Amdahl's Law**: Overall improvement is limited by the part of the system or workflow that must stay sequential.
1. **Murphy's Law**: Possible failures eventually happen, so robust systems assume faults, validate inputs, monitor actively, and recover gracefully.
1. **Postel's Law**: Be precise in what you send and tolerant in what you accept, but apply judgment in security-sensitive contexts.

🔹 **6. How to judge better**
1. **Sturgeon's Law**: Most output in any large body of work is mediocre, so judgment and selection matter as much as creation.
1. **Cunningham's Law**: Wrong answers often attract corrections faster than open-ended questions, but only when the audience is competent.

🔹 **Takeaway**: These laws often pull in different directions, so engineering judgment is not memorizing them, but knowing which one explains the current situation best.
...
🔹 **Vector database**: A store optimized for nearest-neighbor retrieval over embeddings, usually with metadata filters; it supports semantic search and RAG.  
